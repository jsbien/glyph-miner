...
webapp.config.session_parameters = utils.storage({
...
class SessionExpired(webapp.HTTPError): 
    def __init__(self, message):
        webapp.HTTPError.__init__(self, '200 OK', {}, data=message)

class Session(object):
    """Session management for webapp.py
    """
    __slots__ = [
    ...
    def __init__(self, app, store, initializer=None):
        ...
        self._config = utils.storage(webapp.config.session_parameters)
    ...
    def _load(self):
        ...
        self.session_id = webapp.cookies().get(cookie_name)
    ...
    def _validate_ip(self):
        ...
        if self.session_id and self.get('ip', None) != webapp.ctx.ip:
    ...
    def _setcookie(self, session_id, expires='', **kw):
        ...
        webapp.setcookie(cookie_name, session_id, expires=expires, domain=cookie_domain, httponly=httponly, secure=secure, path=cookie_path)
    ...
    def _generate_session_id(self):
        ...
        while True:
            ...
            session_id = sha1("%s%s%s%s" %(rand, now, utils.safestr(webapp.ctx.ip), secret_key))
