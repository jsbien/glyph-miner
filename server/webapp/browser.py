"""Browser to test webapp applications.
(from webapp.py)
"""
...
class Browser:
    ...
    def show(self):
        """Opens the current page in real webapp browser."""
        f = open('page.html', 'w')
        f.write(self.data)
        f.close()
...
class AppBrowser(Browser):
    """Browser interface to test webapp.py apps.
...
class AppHandler(urllib.request.HTTPHandler):
    """urllib2 handler to handle requests using webapp.py application."""
    handler_order = 100
