...
class application:
    ...
    def __init__(self, mapping=(), fvars={}, autoreload=None):
        if autoreload is None:
            autoreload = webapp.config.get('debug', False)
    ...
    def _load(self):
        webapp.ctx.app_stack.append(self)
        
    def _unload(self):
        webapp.ctx.app_stack = webapp.ctx.app_stack[:-1]
        
        if webapp.ctx.app_stack:
            # this is a sub-application, revert ctx to earlier state.
            oldctx = webapp.ctx.get('_oldctx')
            if oldctx:
                webapp.ctx.home = oldctx.home
                webapp.ctx.homepath = oldctx.homepath
                webapp.ctx.path = oldctx.path
                webapp.ctx.fullpath = oldctx.fullpath
    ...
    def request(self, localpart='/', method='GET', data=None,
        ...
        To use https, use https=True.
            ...
            >>> app = application(urls, globals())
            >>> class redirect:
            ...     def GET(self): raise webapp.seeother("/foo")
            ...
        ...
        such as a dict.
            ...
            >>> class uaprinter:
            ...     def GET(self):
            ...         return 'your user-agent is ' + webapp.ctx.env['HTTP_USER_AGENT']
            ... 
            >>> app = application(urls, globals())
    ...
    def handle(self):
        fn, args = self._match(self.mapping, webapp.ctx.path)
        ...
        def process(processors):
            try:
                if processors:
                    ...
                    return self.handle()
            except webapp.HTTPError:
                raise
            except (KeyboardInterrupt, SystemExit):
                raise
            except:
                print(traceback.format_exc(), file=webapp.debug)
    ...
    def wsgifunc(self, *middleware):
        ...
        def wsgi(env, start_resp):
            ...
            try:
                ...
                if webapp.ctx.method.upper() != webapp.ctx.method:
                    raise webapp.nomethod()
...
#                ctx[k] = v.decode('utf-8', 'replace') 
    ...
    def _delegate(self, f, fvars, args=[]):
        def handle_class(cls):
            meth = webapp.ctx.method
            if meth == 'HEAD' and not hasattr(cls, meth):
                meth = 'GET'
            if not hasattr(cls, meth):
                raise webapp.nomethod(cls)
        ...
        if f is None:
            ...
            if f.startswith('redirect '):
                url = f.split(' ', 1)[1]
                if webapp.ctx.method == "GET":
                    x = webapp.ctx.env.get('QUERY_STRING', '')
                    if x:
                        url += '?' + x
                raise webapp.redirect(url)
            elif '.' in f:
                mod, cls = f.rsplit('.', 1)
    ...
    def _delegate_sub_application(self, dir, app):
        ...
        The home, homepath, path and fullpath values in webapp.ctx are updated to mimic request
        to the subapp and are restored after it is handled. 
        
        @@Any issues with when used with yield?
        """
        webapp.ctx._oldctx = webapp.storage(webapp.ctx)
        webapp.ctx.home += dir
        webapp.ctx.homepath += dir
        webapp.ctx.path = webapp.ctx.path[len(dir):]
        webapp.ctx.fullpath = webapp.ctx.fullpath[len(dir):]
        return app.handle_with_processors()
            
    def get_parent_app(self):
        if self in webapp.ctx.app_stack:
            index = webapp.ctx.app_stack.index(self)
            if index > 0:
                return webapp.ctx.app_stack[index-1]
...
class subdomain_application(application):
    ...
    """
    def handle(self):
        host = webapp.ctx.host.split(':')[0] #strip port
