...
config = storage()
config.__doc__ = """
A configuration object for various aspects of webapp.py.
...
def rawinput(method=None):
    ...
    def dictify(fs): 
        # hack to make webapp.input work with enctype='text/plain.
        if fs.list is None:
            fs.list = [] 
    ...
    if method.lower() in ['both', 'post', 'put']:
        if e['REQUEST_METHOD'] in ['POST', 'PUT']:
            if e.get('CONTENT_TYPE', '').lower().startswith('multipart/'):
                ...
                # object in ctx to allow calling webapp.input multiple times.
        ...
        try:
            ...
            for attr_value in http_cookie.split(';'):
                try:
                    ...
                    pass
        cookies = dict((k, urllib.parse.unquote(v.value)) for k, v in cookie.items())
    else:
